...
def remove_first_that_after_adj(sentence, adj):
    ...
    modified_sentence = re.sub(pattern, '', sentence, count=1)

    return modified_sentence

def generate_prompt_14(data, ignore_cache=False, model="gpt-3.5-turbo"):
    user_prompt_list = []
    texts = []
    for index, row in data.iterrows():
        premise = replace_parentheses(row["sentence"])
        without_so = remove_first_that_after_adj(premise, row["adjective"])
        text = remove_so(without_so, row["adjective"])
        texts.append(text)
    ...
    gold_dict = {"causal excess": "bad", "causal": "good", "non-causal": "good", "only causal excess": "bad"}
    ...
    file_path = f"{fold}/result_14.json"
...
if __name__ == "__main__":
    ...
    generate_prompt_14(data, ignore_cache=True, model="gpt-3.5-turbo")
    generate_prompt_14(data, ignore_cache=True, model="gpt-4-0613")
