...
def delete_last_non_alphanumeric_char(input_str):
    # Check if the last character is not alphanumeric
    if not input_str.strip()[-1].isalnum():
        return input_str[:-1].strip()  # Return the string without the last character
    else:
        return input_str.strip()

def generate_prompt_16(data, ignore_cache=False, model="gpt-3.5-turbo"):
    user_prompt_list = []
    for index, row in data.iterrows():
        premise = replace_parentheses(row["sentence"])
        part1, part2 = split_sentence_at_first_that_after_adj(premise, row['adjective'])
        part2 = delete_last_non_alphanumeric_char(part2)
        user_prompt = f'{premise} \nThis entails one of two options. \n1) "{part1} because {part2}" \n2) "{part2.capitalize()} because {part1[0].lower() + part1[1:]}" \nAnswer with the correct number.'
    ...
    gold_dict = {"causal excess": "2", "causal": "1", "non-causal": "not involved", "only causal excess": "2"}
    ...
    file_path = f"{fold}/result_16.json"
...
if __name__ == "__main__":
    ...
    generate_prompt_16(data, ignore_cache=True, model="gpt-3.5-turbo")
    generate_prompt_16(data, ignore_cache=True, model="gpt-4-0613")
