...
def generate_prompt_4(data, ignore_cache=False, model="gpt-3.5-turbo"):
    # creat the prompt
    user_prompt_list = []
    for index, row in data.iterrows():
        premise = replace_parentheses(row["sentence"])
        hypothesis = remove_first_that_after_adj(remove_so(premise, row["adjective"]), row["adjective"])
    ...
    file_path = f"{fold}/result_4.json"
...
if __name__ == "__main__":
    ...
    generate_prompt_4(data, ignore_cache=True, model="gpt-3.5-turbo")
    generate_prompt_4(data, ignore_cache=True, model="gpt-4-0613")
